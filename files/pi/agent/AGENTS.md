Don't use em-dashes and other fancy typopgraphy.

Don't forget to use ^L, when creating big sections inside source code.
Don't remove any existing ^L characters.

Use SPDX headers for license and copyright lines in source code files.
...
use guix shell. There is no /bin/bash, only /bin/sh (which is a symlink to
bash binary in the /gnu/store).  There is no python installed, but guile and
jq are available.

For version control we use jujutsu, so don't rely on git if jj is available
and repo is jj initialized.
...
If there was a substantial change or you noticed a potentially repitetive
pattern, suggest an update for AGENTS.md.
